...
@implementation MSGraphIosStoreAppRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphIosStoreApp *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosStoreApp alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphIosStoreApp *)iosStoreApp
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[iosStoreApp dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:iosStoreApp]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosStoreApp alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
