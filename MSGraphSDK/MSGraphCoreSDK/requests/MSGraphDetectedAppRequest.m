...
@implementation MSGraphDetectedAppRequest
...
- (NSMutableURLRequest *)update:(MSGraphDetectedApp *)detectedApp
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[detectedApp dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:detectedApp]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDetectedApp alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
