...
@implementation MSGraphManagedAppConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphManagedAppConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphManagedAppConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphManagedAppConfiguration *)managedAppConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[managedAppConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:managedAppConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphManagedAppConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
