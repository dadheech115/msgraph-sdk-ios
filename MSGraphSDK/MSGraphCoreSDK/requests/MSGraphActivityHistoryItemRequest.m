...
@implementation MSGraphActivityHistoryItemRequest
...
- (NSMutableURLRequest *)update:(MSGraphActivityHistoryItem *)activityHistoryItem
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[activityHistoryItem dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:activityHistoryItem]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphActivityHistoryItem alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
