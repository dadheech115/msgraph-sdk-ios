...
@implementation MSGraphMobileLobAppRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphMobileLobApp *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphMobileLobApp alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphMobileLobApp *)mobileLobApp
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[mobileLobApp dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:mobileLobApp]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphMobileLobApp alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
