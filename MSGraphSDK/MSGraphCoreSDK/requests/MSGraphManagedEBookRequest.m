...
@implementation MSGraphManagedEBookRequest
...
- (NSMutableURLRequest *)update:(MSGraphManagedEBook *)managedEBook
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[managedEBook dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:managedEBook]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphManagedEBook alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
