...
@implementation MSGraphUserGetManagedAppDiagnosticStatusesRequest
...
- (MSURLSessionDataTask *)executeWithCompletion:(void (^)(MSCollection *response, MSGraphUserGetManagedAppDiagnosticStatusesRequest *nextRequest, NSError *error))completionHandler
{
    ...
    MSURLSessionDataTask *task = [self collectionTaskWithRequest:self.mutableRequest
                                          odObjectWithDictionary:^(id responseObject){
                                                                     return [[MSGraphManagedAppDiagnosticStatus alloc] initWithDictionary:responseObject];
                                                                 }
                                                      completion:^(MSCollection *collectionResponse, NSError *error){
                                      if(!error && collectionResponse.nextLink && completionHandler){
                                              MSGraphUserGetManagedAppDiagnosticStatusesRequest *nextRequest = [[MSGraphUserGetManagedAppDiagnosticStatusesRequest alloc] initWithURL:collectionResponse.nextLink
                                                                                                                  requestOptions:nil
                                                                                                                  client:self.client];
