...
@implementation MSGraphWindows10EndpointProtectionConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWindows10EndpointProtectionConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindows10EndpointProtectionConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWindows10EndpointProtectionConfiguration *)windows10EndpointProtectionConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[windows10EndpointProtectionConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:windows10EndpointProtectionConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindows10EndpointProtectionConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
