...
@implementation MSGraphOutlookCategoryRequest
...
- (NSMutableURLRequest *)update:(MSGraphOutlookCategory *)outlookCategory
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[outlookCategory dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:outlookCategory]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOutlookCategory alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
