...
@implementation MSGraphIosManagedAppRegistrationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphIosManagedAppRegistration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosManagedAppRegistration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphIosManagedAppRegistration *)iosManagedAppRegistration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[iosManagedAppRegistration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:iosManagedAppRegistration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosManagedAppRegistration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
