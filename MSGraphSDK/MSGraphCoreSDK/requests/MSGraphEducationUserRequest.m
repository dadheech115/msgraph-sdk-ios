...
@implementation MSGraphEducationUserRequest
...
- (NSMutableURLRequest *)update:(MSGraphEducationUser *)educationUser
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[educationUser dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:educationUser]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEducationUser alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
