...
@implementation MSGraphDeviceEnrollmentWindowsHelloForBusinessConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphDeviceEnrollmentWindowsHelloForBusinessConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceEnrollmentWindowsHelloForBusinessConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphDeviceEnrollmentWindowsHelloForBusinessConfiguration *)deviceEnrollmentWindowsHelloForBusinessConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[deviceEnrollmentWindowsHelloForBusinessConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:deviceEnrollmentWindowsHelloForBusinessConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceEnrollmentWindowsHelloForBusinessConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
