...
@implementation MSGraphManagedDeviceOverviewRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphManagedDeviceOverview *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphManagedDeviceOverview alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphManagedDeviceOverview *)managedDeviceOverview
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[managedDeviceOverview dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:managedDeviceOverview]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphManagedDeviceOverview alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
