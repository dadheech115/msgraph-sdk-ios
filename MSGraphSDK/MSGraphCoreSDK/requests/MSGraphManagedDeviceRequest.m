...
@implementation MSGraphManagedDeviceRequest
...
- (NSMutableURLRequest *)update:(MSGraphManagedDevice *)managedDevice
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[managedDevice dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:managedDevice]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphManagedDevice alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
