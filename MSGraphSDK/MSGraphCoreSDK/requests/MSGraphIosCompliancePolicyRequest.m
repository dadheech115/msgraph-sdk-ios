...
@implementation MSGraphIosCompliancePolicyRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphIosCompliancePolicy *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosCompliancePolicy alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphIosCompliancePolicy *)iosCompliancePolicy
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[iosCompliancePolicy dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:iosCompliancePolicy]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosCompliancePolicy alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
