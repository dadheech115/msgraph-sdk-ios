...
@implementation MSGraphDeviceEnrollmentLimitConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphDeviceEnrollmentLimitConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceEnrollmentLimitConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphDeviceEnrollmentLimitConfiguration *)deviceEnrollmentLimitConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[deviceEnrollmentLimitConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:deviceEnrollmentLimitConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceEnrollmentLimitConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
