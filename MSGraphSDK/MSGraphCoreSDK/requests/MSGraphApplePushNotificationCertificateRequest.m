...
@implementation MSGraphApplePushNotificationCertificateRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphApplePushNotificationCertificate *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphApplePushNotificationCertificate alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphApplePushNotificationCertificate *)applePushNotificationCertificate
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[applePushNotificationCertificate dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:applePushNotificationCertificate]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphApplePushNotificationCertificate alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
