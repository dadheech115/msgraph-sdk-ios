...
@implementation MSGraphIosUpdateConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphIosUpdateConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosUpdateConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphIosUpdateConfiguration *)iosUpdateConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[iosUpdateConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:iosUpdateConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosUpdateConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
