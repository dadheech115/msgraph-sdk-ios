...
@implementation MSGraphDirectoryRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphDirectory *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDirectory alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphDirectory *)directory
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[directory dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:directory]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDirectory alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
