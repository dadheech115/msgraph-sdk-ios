...
@implementation MSGraphDeviceEnrollmentPlatformRestrictionsConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphDeviceEnrollmentPlatformRestrictionsConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceEnrollmentPlatformRestrictionsConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphDeviceEnrollmentPlatformRestrictionsConfiguration *)deviceEnrollmentPlatformRestrictionsConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[deviceEnrollmentPlatformRestrictionsConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:deviceEnrollmentPlatformRestrictionsConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceEnrollmentPlatformRestrictionsConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
