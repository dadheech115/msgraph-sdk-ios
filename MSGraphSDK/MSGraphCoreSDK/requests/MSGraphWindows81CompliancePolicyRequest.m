...
@implementation MSGraphWindows81CompliancePolicyRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWindows81CompliancePolicy *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindows81CompliancePolicy alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWindows81CompliancePolicy *)windows81CompliancePolicy
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[windows81CompliancePolicy dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:windows81CompliancePolicy]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindows81CompliancePolicy alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
