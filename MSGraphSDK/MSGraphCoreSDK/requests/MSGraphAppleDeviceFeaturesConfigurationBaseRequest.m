...
@implementation MSGraphAppleDeviceFeaturesConfigurationBaseRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphAppleDeviceFeaturesConfigurationBase *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphAppleDeviceFeaturesConfigurationBase alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphAppleDeviceFeaturesConfigurationBase *)appleDeviceFeaturesConfigurationBase
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[appleDeviceFeaturesConfigurationBase dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:appleDeviceFeaturesConfigurationBase]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphAppleDeviceFeaturesConfigurationBase alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
