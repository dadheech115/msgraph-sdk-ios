...
@implementation MSGraphUserActivityRequest
...
- (NSMutableURLRequest *)update:(MSGraphUserActivity *)userActivity
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[userActivity dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:userActivity]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphUserActivity alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
