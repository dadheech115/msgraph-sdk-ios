...
@implementation MSGraphRoleAssignmentRequest
...
- (NSMutableURLRequest *)update:(MSGraphRoleAssignment *)roleAssignment
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[roleAssignment dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:roleAssignment]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphRoleAssignment alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
