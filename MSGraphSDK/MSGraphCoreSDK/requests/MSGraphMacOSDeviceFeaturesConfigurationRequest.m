...
@implementation MSGraphMacOSDeviceFeaturesConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphMacOSDeviceFeaturesConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphMacOSDeviceFeaturesConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphMacOSDeviceFeaturesConfiguration *)macOSDeviceFeaturesConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[macOSDeviceFeaturesConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:macOSDeviceFeaturesConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphMacOSDeviceFeaturesConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
