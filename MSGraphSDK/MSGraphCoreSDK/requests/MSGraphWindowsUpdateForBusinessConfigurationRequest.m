...
@implementation MSGraphWindowsUpdateForBusinessConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWindowsUpdateForBusinessConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindowsUpdateForBusinessConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWindowsUpdateForBusinessConfiguration *)windowsUpdateForBusinessConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[windowsUpdateForBusinessConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:windowsUpdateForBusinessConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindowsUpdateForBusinessConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
