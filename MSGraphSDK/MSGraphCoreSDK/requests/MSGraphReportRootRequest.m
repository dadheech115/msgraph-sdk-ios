...
@implementation MSGraphReportRootRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphReportRoot *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphReportRoot alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphReportRoot *)reportRoot
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[reportRoot dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:reportRoot]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphReportRoot alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
