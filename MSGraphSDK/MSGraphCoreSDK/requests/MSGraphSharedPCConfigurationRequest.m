...
@implementation MSGraphSharedPCConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphSharedPCConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSharedPCConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphSharedPCConfiguration *)sharedPCConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[sharedPCConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:sharedPCConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSharedPCConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
