...
@implementation MSGraphDeviceManagementRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphDeviceManagement *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceManagement alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphDeviceManagement *)deviceManagement
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[deviceManagement dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:deviceManagement]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceManagement alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
