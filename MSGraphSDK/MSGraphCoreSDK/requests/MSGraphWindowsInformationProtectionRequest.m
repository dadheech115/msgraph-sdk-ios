...
@implementation MSGraphWindowsInformationProtectionRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWindowsInformationProtection *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindowsInformationProtection alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWindowsInformationProtection *)windowsInformationProtection
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[windowsInformationProtection dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:windowsInformationProtection]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindowsInformationProtection alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
