...
@implementation MSGraphWebAppRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWebApp *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWebApp alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWebApp *)webApp
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[webApp dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:webApp]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWebApp alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
