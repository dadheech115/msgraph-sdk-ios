...
@implementation MSGraphVppTokenRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphVppToken *response, NSError *error))completionHandler
{
...
- (NSMutableURLRequest *)update:(MSGraphVppToken *)vppToken
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[vppToken dictionaryFromItem] options:0 error:nil];
    return [self requestWithMethod:@"PATCH"
                              body:body
                           headers:nil];
}

- (MSURLSessionDataTask *)update:(MSGraphVppToken *)vppToken withCompletion:(void (^)(MSGraphVppToken *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:vppToken]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphVppToken alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
