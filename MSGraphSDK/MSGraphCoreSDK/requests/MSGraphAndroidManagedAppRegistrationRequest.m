...
@implementation MSGraphAndroidManagedAppRegistrationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphAndroidManagedAppRegistration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphAndroidManagedAppRegistration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphAndroidManagedAppRegistration *)androidManagedAppRegistration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[androidManagedAppRegistration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:androidManagedAppRegistration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphAndroidManagedAppRegistration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
