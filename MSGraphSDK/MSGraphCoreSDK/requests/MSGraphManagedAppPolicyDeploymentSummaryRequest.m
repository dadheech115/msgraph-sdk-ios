...
@implementation MSGraphManagedAppPolicyDeploymentSummaryRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphManagedAppPolicyDeploymentSummary *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphManagedAppPolicyDeploymentSummary alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphManagedAppPolicyDeploymentSummary *)managedAppPolicyDeploymentSummary
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[managedAppPolicyDeploymentSummary dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:managedAppPolicyDeploymentSummary]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphManagedAppPolicyDeploymentSummary alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
