...
@implementation MSGraphWindows10CustomConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWindows10CustomConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindows10CustomConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWindows10CustomConfiguration *)windows10CustomConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[windows10CustomConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:windows10CustomConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindows10CustomConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
