...
@implementation MSGraphMacOSGeneralDeviceConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphMacOSGeneralDeviceConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphMacOSGeneralDeviceConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphMacOSGeneralDeviceConfiguration *)macOSGeneralDeviceConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[macOSGeneralDeviceConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:macOSGeneralDeviceConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphMacOSGeneralDeviceConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
