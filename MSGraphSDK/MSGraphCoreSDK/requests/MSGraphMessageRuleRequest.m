...
@implementation MSGraphMessageRuleRequest
...
- (NSMutableURLRequest *)update:(MSGraphMessageRule *)messageRule
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[messageRule dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:messageRule]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphMessageRule alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
