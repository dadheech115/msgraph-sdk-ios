...
@implementation MSGraphDeviceConfigurationUserOverviewRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphDeviceConfigurationUserOverview *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceConfigurationUserOverview alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphDeviceConfigurationUserOverview *)deviceConfigurationUserOverview
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[deviceConfigurationUserOverview dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:deviceConfigurationUserOverview]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDeviceConfigurationUserOverview alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
