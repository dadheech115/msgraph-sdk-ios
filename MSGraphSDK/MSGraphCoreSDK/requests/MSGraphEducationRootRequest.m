...
@implementation MSGraphEducationRootRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphEducationRoot *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEducationRoot alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphEducationRoot *)educationRoot
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[educationRoot dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:educationRoot]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEducationRoot alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
