...
@implementation MSGraphMobileAppRequest
...
- (NSMutableURLRequest *)update:(MSGraphMobileApp *)mobileApp
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[mobileApp dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:mobileApp]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphMobileApp alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
