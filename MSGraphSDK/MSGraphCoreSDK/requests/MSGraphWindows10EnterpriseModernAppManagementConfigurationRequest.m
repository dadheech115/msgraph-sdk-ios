...
@implementation MSGraphWindows10EnterpriseModernAppManagementConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWindows10EnterpriseModernAppManagementConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindows10EnterpriseModernAppManagementConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWindows10EnterpriseModernAppManagementConfiguration *)windows10EnterpriseModernAppManagementConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[windows10EnterpriseModernAppManagementConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:windows10EnterpriseModernAppManagementConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindows10EnterpriseModernAppManagementConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
