...
@implementation MSGraphWindowsPhone81GeneralConfigurationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWindowsPhone81GeneralConfiguration *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindowsPhone81GeneralConfiguration alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWindowsPhone81GeneralConfiguration *)windowsPhone81GeneralConfiguration
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[windowsPhone81GeneralConfiguration dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:windowsPhone81GeneralConfiguration]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWindowsPhone81GeneralConfiguration alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
