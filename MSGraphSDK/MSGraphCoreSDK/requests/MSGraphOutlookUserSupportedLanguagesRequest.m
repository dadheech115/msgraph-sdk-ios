...
@implementation MSGraphOutlookUserSupportedLanguagesRequest
...
- (MSURLSessionDataTask *)executeWithCompletion:(void (^)(MSCollection *response, MSGraphOutlookUserSupportedLanguagesRequest *nextRequest, NSError *error))completionHandler
{
    ...
    MSURLSessionDataTask *task = [self collectionTaskWithRequest:self.mutableRequest
                                          odObjectWithDictionary:^(id responseObject){
                                                                     return [[MSGraphLocaleInfo alloc] initWithDictionary:responseObject];
                                                                 }
                                                      completion:^(MSCollection *collectionResponse, NSError *error){
                                      if(!error && collectionResponse.nextLink && completionHandler){
                                              MSGraphOutlookUserSupportedLanguagesRequest *nextRequest = [[MSGraphOutlookUserSupportedLanguagesRequest alloc] initWithURL:collectionResponse.nextLink
                                                                                                                  requestOptions:nil
                                                                                                                  client:self.client];
