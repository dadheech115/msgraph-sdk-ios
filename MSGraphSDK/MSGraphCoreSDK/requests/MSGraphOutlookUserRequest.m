...
@implementation MSGraphOutlookUserRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphOutlookUser *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOutlookUser alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphOutlookUser *)outlookUser
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[outlookUser dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:outlookUser]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOutlookUser alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
