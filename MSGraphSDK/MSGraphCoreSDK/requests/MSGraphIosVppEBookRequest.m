...
@implementation MSGraphIosVppEBookRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphIosVppEBook *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosVppEBook alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphIosVppEBook *)iosVppEBook
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[iosVppEBook dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:iosVppEBook]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIosVppEBook alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
