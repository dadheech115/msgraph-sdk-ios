...
- (MSDuration*) periodOfflineBeforeAccessCheck
{
    if(!_periodOfflineBeforeAccessCheck){
        _periodOfflineBeforeAccessCheck = self.dictionary[@"periodOfflineBeforeAccessCheck"];
    }
...
- (MSDuration*) periodOnlineBeforeAccessCheck
{
    if(!_periodOnlineBeforeAccessCheck){
        _periodOnlineBeforeAccessCheck = self.dictionary[@"periodOnlineBeforeAccessCheck"];
    }
...
- (MSDuration*) periodOfflineBeforeWipeIsEnforced
{
    if(!_periodOfflineBeforeWipeIsEnforced){
        _periodOfflineBeforeWipeIsEnforced = self.dictionary[@"periodOfflineBeforeWipeIsEnforced"];
    }
...
- (MSDuration*) periodBeforePinReset
{
    if(!_periodBeforePinReset){
        _periodBeforePinReset = self.dictionary[@"periodBeforePinReset"];
    }
...
- (NSArray*) allowedDataStorageLocations
{
    ...
    if ([allowedDataStorageLocations isKindOfClass:[NSArray class]]){
        for (id managedAppDataStorageLocation in allowedDataStorageLocations){
            [allowedDataStorageLocationsResult addObject:[managedAppDataStorageLocation toMSGraphManagedAppDataStorageLocation]];
        }
    }
